...
const TAILWIND_UTILITY = "bg|text|border|ring|from|to|via|fill|stroke|outline|decoration|accent|caret";
const RUNE_PATTERN = /\$(state|derived|effect)\b/g;
const IMPORT_PATTERN = /\b(?:import|export)\s+(?:type\s+)?(?:[^"']*?\s+from\s*)?["']([^"']+)["']|import\s*\(\s*["']([^"']+)["']\s*\)/g;
  ...
  lines.forEach((line, index) => {
    for (const match of line.matchAll(TAILWIND_PATTERN)) {
      violations.push({
        ...
        line: index + 1,
        match: match[1],
        message: "Cor padrao do Tailwind detectada. Use tokens definidos no styleguide.md."
      });
    }
  });
...
export function resolveFeatureImport(specifier, importerRelativePath) {
  ...
  const srcIndex = parts.findIndex((part, index) => part === "src" && parts[index + 1] === "features");
  ...
  const privateSegment = rest.find((segment) => FEATURE_PRIVATE_SEGMENTS.has(stripExtension(segment)));
