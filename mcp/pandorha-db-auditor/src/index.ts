...
const SAFE_PRAGMA =
	/^pragma\s+(table_info|table_xinfo|table_list|index_list|index_info|foreign_key_list|database_list)\s*\(/i;
const BLOCKED_SQL =
	/\b(insert|update|delete|drop|alter|create|replace|attach|detach|vacuum|reindex|analyze|truncate)\b/i;

export function resolveDbPath(env: NodeJS.ProcessEnv = process.env): string {
	if (env.PANDORHA_DB_PATH) return path.resolve(env.PANDORHA_DB_PATH);

	const projectRoot = path.resolve(
		env.PANDORHA_PROJECT_ROOT || DEFAULT_PROJECT_ROOT,
	);
	...
	return {
		...
		verifyMath(actorId: string | number) {
			...
			return {
				...
				hp: {
					...
					calculated: hp,
					ok:
						hp !== null && storedHp !== null
							? numbersMatch(storedHp, hp)
							: false,
				},
				ee: {
					stored: storedEe,
					calculated: ee,
					ok:
						ee !== null && storedEe !== null
							? numbersMatch(storedEe, ee)
							: null,
				},
				ok:
					missing.length === 0 &&
		...
		executeQuery(sql: string, params: QueryParams = []) {
			...
			if (!isReadOnlySql(cleanSql)) {
				throw new Error(
					"execute_query accepts only read-only SELECT, WITH, or safe schema PRAGMA statements",
				);
			}
...
export function buildColumnMap(columns: string[]): ColumnMap {
	...
	const map: ColumnMap = {
		...
		applications: {
			...
			interacao: pick(lookup, [
				"interacao",
				"interaction",
				"aplicacao_interacao",
			]),
			resistencia: pick(lookup, [
				"resistencia",
				"res",
				"resistance",
				"aplicacao_resistencia",
			]),
		},
		matrix: {},
	};
...
function findActor(
	...
	if (selector.actor_name) {
		if (!map.name)
			throw new Error(
				"Cannot search by actor_name because no name column was detected",
			);
		const row = db
			.prepare(
