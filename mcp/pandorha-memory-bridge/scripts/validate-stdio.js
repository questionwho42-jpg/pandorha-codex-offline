...
const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pandorha-memory-bridge-stdio-"));
  ...
  for (const line of lines) {
    ...
    if (message.id === 2 && message.result) {
      ...
      send({
        ...
        method: "tools/call",
        params: {
          name: "commit_module_context",
          arguments: {
            module_name: "Validation Module",
            error_log: "STDIO validation executed successfully.",
            technical_summary: "The bridge created module context files in a temporary project root.",
            plain_english: "This confirms the memory bridge can write the three context files."
          }
        }
      });
      continue;
...
send({
  ...
  params: {
    ...
    capabilities: {},
    clientInfo: {
      name: "pandorha-memory-bridge-validator",
      version: "0.1.0"
    }
  }
});
