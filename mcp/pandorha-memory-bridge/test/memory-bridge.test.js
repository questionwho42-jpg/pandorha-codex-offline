...
test("resolveContextDir stays under src/features", () => {
	...
	assert.equal(
		result.contextDir,
		path.join(projectRoot, "src", "features", "social-layer", ".context"),
	);
});
...
test("resolveContextDir can target src/entities explicitly", () => {
	...
	assert.equal(
		result.contextDir,
		path.join(projectRoot, "src", "entities", "ancestry", ".context"),
	);
});
...
test("commitModuleContext creates the three context files", async () => {
	...
	const result = await commitModuleContext(sampleInput(), {
		projectRoot,
		now: fixedDate,
	});
	...
	assert.equal(
		result.files.every((file) => file.operation === "created"),
		true,
	);

	const tech = await fs.readFile(
		path.join(contextDir, "tech-memory.md"),
		"utf8",
	);
	const scaling = await fs.readFile(
		path.join(contextDir, "scaling-roadmap.md"),
		"utf8",
	);
	const plain = await fs.readFile(
		path.join(contextDir, "plain-english.md"),
		"utf8",
	);
...
test("commitModuleContext creates entity context files when layer is entities", async () => {
	const projectRoot = await createTempProjectRoot();
	const result = await commitModuleContext(
		sampleInput({ module_name: "Equipment", layer: "entities" }),
		{
			projectRoot,
			now: fixedDate,
		},
	...
	assert.equal(result.files.length, 3);
	assert.match(
		await fs.readFile(path.join(contextDir, "tech-memory.md"), "utf8"),
		/Service writes context files/,
	);
});

test("commitModuleContext appends without deleting previous entries", async () => {
	const projectRoot = await createTempProjectRoot();
	await commitModuleContext(sampleInput({ error_log: "Primeira entrada." }), {
		projectRoot,
		now: fixedDate,
	});
	const result = await commitModuleContext(
		sampleInput({ error_log: "Segunda entrada." }),
		{ projectRoot, now: fixedDate },
	);
	...
	assert.equal(
		result.files.every((file) => file.operation === "updated"),
		true,
	);
...
test("commitModuleContext uses explicit scaling notes when provided", async () => {
	const projectRoot = await createTempProjectRoot();
	await commitModuleContext(
		sampleInput({ scaling_notes: "Criar cache por modulo." }),
		{ projectRoot, now: fixedDate },
	);
...
test("renderers provide defaults for empty optional content", () => {
	...
	assert.match(
		renderTechMemoryEntry(fixedDate.toISOString(), empty),
		/No details provided/,
	);
	assert.match(
		renderPlainEnglishEntry(fixedDate.toISOString(), empty),
		/No details provided/,
	);
	assert.match(
		renderScalingRoadmapEntry(fixedDate.toISOString(), empty),
		/Review this module/,
	);
});
...
test("resolveProjectRoot honors env override", () => {
	assert.equal(
		resolveProjectRoot({ PANDORHA_PROJECT_ROOT: "C:/tmp/pandorha" }),
		path.resolve("C:/tmp/pandorha"),
	);
});
