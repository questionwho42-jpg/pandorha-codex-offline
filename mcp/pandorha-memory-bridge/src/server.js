...
export async function commitModuleContext(input, options = {}) {
	...
	const { moduleSlug, contextDir } = resolveContextDir(
		projectRoot,
		input.module_name,
		input.layer,
	);
	...
	server.tool(
		"commit_module_context",
		{
			module_name: z
				.string()
				.min(1)
				.describe("Module name under the selected src layer."),
			layer: z
				.enum(["features", "entities"])
				.optional()
				.describe("Context layer under src; defaults to features."),
			error_log: z.string().describe("Errors found and fixes applied."),
			technical_summary: z
				.string()
				.describe("Technical decisions, APIs, and conventions."),
			plain_english: z
				.string()
				.describe("Plain-language explanation for non-technical readers."),
			scaling_notes: z
				.string()
				.optional()
				.describe("Optional scalability notes and future work."),
		},
		async (input) =>
			jsonText(await commitModuleContext(input, { projectRoot })),
	);
