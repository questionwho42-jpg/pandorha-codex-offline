...
import { walkMarkdownFiles, readUtf8File } from "../src/file-system.js";
import { buildMarkdownSegments } from "../src/markdown-segments.js";
import { createSearchEngine, loadKnowledgeIndex } from "../src/search-engine.js";

const projectRoot = path.resolve("C:/pandorha-test");
const filePath = path.join(projectRoot, "docs", "system", "combat", "damage.md");
const markdown = `---
...
test("buildMarkdownSegments extracts headings and tables", () => {
  ...
  const heading = segments.find((segment) => segment.kind === "heading" && segment.title === "Codex de Combate");

  assert.ok(heading);
  assert.equal(heading.content.includes("<ai_context>"), false);
  assert.equal(segments.some((segment) => segment.content.includes("sourceFile")), false);
  assert.ok(segments.some((segment) => segment.kind === "table" && segment.content.includes("Dano Cortante")));
});

test("searchRpgRule prioritizes table matches", () => {
  const segments = buildMarkdownSegments(filePath, markdown, { projectRoot });
  const engine = createSearchEngine(segments, {
    stats: { files: 1, segments: segments.length, roots: [path.join(projectRoot, "docs")] }
  });
...
test("walkMarkdownFiles rethrows non-missing filesystem errors", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pandorha-invalid-root-"));
  const notDirectory = path.join(root, "file.md");

  await fs.writeFile(notDirectory, "# not a directory", "utf8");

  await assert.rejects(() => walkMarkdownFiles([notDirectory]), /ENOTDIR|not a directory|readdir/);
});
...
test("loadKnowledgeIndex builds a searchable index from disk", async () => {
  ...
  await fs.writeFile(path.join(docs, "rules.md"), [
    "# Regras de Acao",
    "",
...
test("searchRpgRule removes duplicate results from the same source line", () => {
  ...
  const engine = createSearchEngine([
    { ...shared, id: "heading", kind: "heading", content: "# Dano\nRegra de dano." },
    { ...shared, id: "section", kind: "section", content: "# Dano\nRegra de dano detalhada." }
  ]);
...
test("large sections are split into bounded chunks", () => {
  ...
  assert.ok(segments.filter((segment) => segment.kind === "section").length > 1);
