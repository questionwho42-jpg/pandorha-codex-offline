...
import { readUtf8File, walkMarkdownFiles } from "./file-system.js";
...
export async function loadKnowledgeIndex(config) {
	...
	for (const filePath of files) {
		...
		segments.push(
			...buildMarkdownSegments(filePath, markdown, {
...
export function createSearchEngine(segments, options = {}) {
	...
	return {
		...
		searchRpgRule(term, searchOptions = {}) {
			...
			const ranked = fuse
				.search(cleanTerm, { limit: Math.max(limit * 4, limit) })
...
function matchesTokenRequirement(segment, term) {
	...
	const tokenPattern = new RegExp(
		`(^|[^\\p{L}\\p{N}])${escapeRegExp(tokens[0])}(?=[^\\p{L}\\p{N}]|$)`,
		"u",
	);
