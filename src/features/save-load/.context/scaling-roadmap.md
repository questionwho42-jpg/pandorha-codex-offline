...
- Keep v9 limited to inventory ownership, equipment loadout events, character trait selections, and equipment durability events; real HP, trait effects, editable sheet state, automatic wear, repair costs, or additional social history require later version gates.
...
- Keep character trait selections in the same snapshot transaction as characters so restored sheets never expose orphaned trait choices.
- Keep equipment durability events in the same transaction as inventory events and validate them before exposing loaded state; future condition-derived modifiers should live outside save/load.
