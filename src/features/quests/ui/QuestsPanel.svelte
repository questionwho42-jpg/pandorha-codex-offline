...
import type { QuestRepository } from "$lib/entities/quest/domain/QuestRepository";
...
let {
	repository = new WorkerQuestRepository(),
...
async function loadQuestsFromStore() {
	...
	if (res.success) {
		...
		// Carregar objetivos de cada quest ativa
		const objectivesMap: Record<string, QuestObjectiveRecord[]> = {};
		for (const q of activeQuests) {
			const objRes = await service.listObjectives(q.id);
			if (objRes.success) {
				objectivesMap[q.id] = objRes.data;
			}
		}
...
// biome-ignore lint/correctness/noUnusedVariables: consumed by Svelte markup
async function handleCompleteObjective(objectiveId: string) {
	const res = await service.updateObjectiveProgress(objectiveId, 1);
	if (res.success) {
		await loadQuestsFromStore();
		triggerSuccess("Objetivo atualizado/concluído!");
	} else {
		triggerError(res.error.message);
	}
}
...
async function handleCreateTestQuest() {
	...
	if (res.success) {
		// Criar objetivo no Hexcrawl
		await service.addObjective(
			testId,
			`obj_${testId}_1`,
			"Explorar as Ruínas Baixas no Hexcrawl (southeast-ruins)",
			"custom",
			"southeast-ruins",
			1,
		);
		// Criar objetivo na Masmorra
		await service.addObjective(
			testId,
			`obj_${testId}_2`,
			"Derrotar o Guardião Mecânico no Bastião (Sala 3,3)",
			"custom",
			"room_3_3",
			1,
		);
		...
		await loadQuestsFromStore();
		triggerSuccess("Nova missão com objetivos de mapa integrada!");
	} else {
		triggerError(res.error.message);
	}
}
...
<div class="quest-log-container">
	{#if successNotification}
		<div class="alert success-alert animate-fade" data-testid="success-notification">
			<span>✔️ {successNotification}</span>
		</div>
	{/if}
	{#if errorNotification}
		<div class="alert error-alert animate-fade" data-testid="error-notification">
			<span>⚠️ {errorNotification}</span>
		</div>
	{/if}

	<div class="panel header-panel">
		<h1>Diário de Missões & Objetivos</h1>
		<p class="subtitle">O escriba registra cada passo na escuridão; o diário é a prova de que lutamos.</p>

		<div class="tabs-header">
			<button class="tab-btn" class:active={activeTab === "active"} onclick={() => activeTab = "active"} data-testid="tab-active">
				Missões Ativas ({activeQuests.length})
			</button>
			<button class="tab-btn" class:active={activeTab === "completed"} onclick={() => activeTab = "completed"} data-testid="tab-completed">
				Histórico ({completedQuests.length})
			</button>
			<button class="tab-btn" class:active={activeTab === "rumors"} onclick={() => activeTab = "rumors"} data-testid="tab-rumors">
				Boatos & Rumores ({rumors.length})
			</button>
		</div>

		{#if activeTab === "active"}
			<div class="quest-list" data-testid="active-quest-list">
				{#each activeQuests as q (q.id)}
					{@const r = getRewards(q.rewardsJson)}
					{@const reqs = getRequirements(q.requirementsJson)}
					{@const objectives = questObjectivesMap[q.id] || []}
					<div class="quest-card" data-testid={`quest-card-${q.id}`}>
						...
						<!-- Objetivos Dinâmicos Rúnicos -->
						<div class="objectives-section mt-4 border-t border-bronze/10 pt-3">
							<h4 class="text-xs font-bold text-ether uppercase tracking-wider mb-2">Objetivos da Missão</h4>
							{#if objectives.length === 0}
								<p class="text-xs text-bone/50 italic">Nenhum objetivo específico registrado.</p>
							{:else}
								<div class="objectives-list flex flex-col gap-2">
									{#each objectives as obj (obj.id)}
										<div class="objective-row flex items-center justify-between gap-3 p-2 bg-void/50 border border-bronze/10 rounded" class:completed={obj.status === "completed"}>
											<div class="flex items-center gap-2">
												<span class="status-indicator">
													{obj.status === "completed" ? "✔️" : "⭕"}
												</span>
												<div class="flex flex-col">
													<span class="objective-text text-xs text-bone/90" class:line-through={obj.status === "completed"}>
														{obj.description}
													</span>
													<span class="text-[9px] text-ether/60 uppercase">
														Alvo: {obj.target} · Progresso: {obj.currentAmount}/{obj.requiredAmount}
													</span>
												</div>
											</div>
											{#if obj.status !== "completed"}
												<button 
													class="btn-objective-complete text-[10px] px-2 py-0.5 border border-sky-runic/50 hover:bg-sky-runic/20 text-sky-runic rounded"
													onclick={() => handleCompleteObjective(obj.id)}
													data-testid={`complete-obj-${obj.id}`}
												>
													Marcar Progresso
												</button>
											{/if}
						...
						<div class="quest-meta mt-4 border-t border-bronze/10 pt-3">
							{#if reqs.length > 0}
						...
						<div class="quest-actions">
							<button class="btn btn-sm btn-primary" onclick={() => handleCompleteQuest(q.id)} data-testid={`complete-quest-${q.id}`}>Concluir Missão</button>
							<button class="btn btn-sm btn-danger" onclick={() => handleFailQuest(q.id)} data-testid={`fail-quest-${q.id}`}>Falhar</button>
						</div>
					</div>
				{/each}

				{#if activeQuests.length === 0}
					<p class="empty">Nenhuma missão ativa no diário. Procure Silas ou Silas de Alvorada por boatos.</p>
				{/if}
			</div>
		...
		<div class="dev-controls">
			<button class="btn btn-outline" onclick={handleCreateTestQuest} data-testid="add-test-quest-btn">
				📜 Adicionar Missão de Teste com Objetivos
			</button>
		</div>
	</div>
</div>
