...
import SocialStandings from "./SocialStandings.svelte";
...
let selectedAxis = $state("Social + Interacao");
// biome-ignore lint/correctness/noUnusedVariables: consumed by Svelte markup.
let simulatedMargin = $state(5);
// biome-ignore lint/correctness/noUnusedVariables: consumed by Svelte markup.
let selectedManeuver = $state<SocialManeuverType>("none");
</script>
