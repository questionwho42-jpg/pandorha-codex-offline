...
export const characters = sqliteTable("characters", {
	...
	tensionMeter: integer("tension_meter").notNull().default(0),
...
export const characterInsertSchema = createInsertSchema(characters).extend({
	...
	tensionMeter: z.number().int().min(0).max(100).optional().default(0),
...
export const characterStatusEffectInsertSchema = createInsertSchema(
	...
	durationTurns: z.number().int().nullable().optional(),
	createdAt: notBlankText,
...
export const characterStatusEffectSelectSchema = createSelectSchema(
	...
	severity: z.number().int().min(1).max(20),
	severityMax: z.number().int().min(1).max(20),
...
export const levelUpInputSchema = z.object({
	characterId: notBlankText,
	addedPhysical: z.number().int().min(0).max(1).default(0),
	addedMental: z.number().int().min(0).max(1).default(0),
	addedSocial: z.number().int().min(0).max(1).default(0),
	addedConflict: z.number().int().min(0).max(2).default(0),
	addedInteraction: z.number().int().min(0).max(2).default(0),
	addedResistance: z.number().int().min(0).max(2).default(0),
});
