...
- A auditoria renderizada no Browser do Codex permanece bloqueada porque esta sessão não expôs Browser Plugin, handle de página, console ou endpoint local de depuração utilizável. O aceite renderizado deve ser repetido quando o controle da aba estiver disponível.
...
| Validação renderizada pendente | Repetir o fluxo completo no Browser do Codex quando a aba expuser handle de página e console ao agente. |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| Loadout persistente por personagem | A aba persiste ownership e quantidades, mas ainda não persiste quais itens estão equipados. | Contrato de loadout por personagem, ownership aprovado, `EquipmentLoadoutService`, save e migration próprios. | Depois que o inventário editável passar pelo aceite renderizado; antes de ações de equipar. | Alto: pode duplicar ownership ou estado derivado no save. | `inventory-management`/`equipment`; inbox `20260615-future-inventory-persistent-loadout`. |
...
1. Repetir o aceite renderizado no Browser do Codex quando a aba expuser handle de página e console ao agente.
2. Implementar loadout persistente e depois equipar/desequipar.
3. Integrar o loadout persistido ao combate antes de durabilidade ou cinto de poções.
4. Entregar equipamento inicial somente junto ao contrato completo de ficha.
5. Persistir traços somente junto ao contrato completo de ficha e migration.
6. Ampliar Compêndio após pipeline de ingestão, pois ele reduz dependência de consulta manual às regras.
7. Implementar Acampamento multi-hora e Relações superiores apenas após seus contratos explícitos.
8. Manter execução de Magia e HP real persistido para fases posteriores, pois possuem maior risco de regra, save e integração.
9. Criar painel de WorldState e UI PWA quando houver demanda recorrente e contratos de visibilidade/atualização definidos.
