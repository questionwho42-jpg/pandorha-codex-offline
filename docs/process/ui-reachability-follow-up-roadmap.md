...
- A rodada economica de 2026-06-21 confirmou `quality:root` fora do sandbox com 804 testes, `quality:automation`, `quality:mcp`, `quality:skills`, `qa:vertical-slice`, `qa:ui-reachability`, `qa:social-browser-smoke`, `qa:dialogue-seeds`, `compendium:generate:check`, `context:validate`, `save:migration-matrix` e `qa:browser-runbook:check`.
...
| Limitações deliberadas | Combate ainda usa alvos de treino, HP de treino local e não persiste dano real; magia sem execução, Compendio sem busca semantica/lore extensa, Acampamento sem noite automatica e relações Tier 1 permanecem limites deliberados. |
...
| Perfis completos dos itens de kit inicial | Personagens novos recebem kits de classe pelo ledger de inventário, mas `chainmail`, `shortbow`, `staff`, `rapier` e `luxury-padded-armor` nao atingiram o criterio de evidencia completa em `docs/process/starting-equipment-profile-evidence-gate.md`. | Gate `docs/process/starting-equipment-loadout-profile-gate.md`, gate de evidencia, fonte soberana para dado/CA/tags/maos/penalidades e validação contra loadout persistido. | Somente depois de aprovar estatísticas completas, substituições explícitas ou um gate separado de foco magico para `staff`. | Alto: pode tornar itens equipáveis com bônus inferidos. | `equipment`/`inventory-management`/`combat-encounter`; inbox `20260618-065745-starting-equipment-catalog-implementation` e `20260619-175238-starting-equipment-profile-gap-gate`. |
...
| Noite completa de Acampamento | A UI agora prepara horas seguintes manualmente e preserva perigo/relogio no save v9, mas nao automatiza uma noite nem mantem historico completo de horas. | Novo gate para quantidade de horas, interrupcao, historico, atividades avancadas e politica de reset. | Depois que o encadeamento manual permanecer estavel e houver regras soberanas suficientes. | Alto: pode confundir clocks, restauracao e efeitos de atividades. | `camp-hour`; inbox `20260606-future-camp-multi-hour`. |
...
1. Expandir perfis dos itens de kit inicial somente após os gates `starting-equipment-loadout-profile-gate` e `starting-equipment-profile-evidence-gate` serem satisfeitos por estatísticas soberanas completas, substituições explícitas ou gate separado de foco magico para `staff`.
...
4. Implementar noite completa de Acampamento e Relações superiores apenas após seus contratos explícitos.
