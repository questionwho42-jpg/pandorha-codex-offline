...
Este roteiro valida o MVP navegável atual do Pandorha Engine depois da T64. Ele não substitui `quality:gate`; ele descreve o caminho humano que deve ser repetido antes de expandir diálogo literário completo ou consequências narrativas mais profundas.
...
12. Selecione `Capitão de Treino`, clique em `Reiniciar negociação`, confirme que a fala cita `moral da tropa`, escolha `Barganhar` e confirme a resposta sobre `custo da escolta`.
...
14. Para validar T63, reinicie a negociação com a `Corretora de Treino`, escolha `Pressionar`, faça apelos até encerrar a conversa e confirme consequência com `perda de 1 nível de Fama` e `Fama` reduzida na `Liga Mercante de Treino`.
15. Clique em `Salvar sessão`, recarregue, clique em `Carregar save` e confirme que relações, negociação social, log com argumento, opção bloqueada, consequência com a escolha de diálogo e `Fama` reduzida foram restaurados.
...
- catálogo `training-captain` com opção `Pressionar` bloqueável por `minimumMentalHp` 8 e copy oficial curta sobre moral da tropa e custo da escolta;
...
- contrato T63 de `Pressionar` com flag `social-pressure-fame-penalty`, callback de penalidade e perda de 1 nível de `Fama`;
...
Esse comando executa `scripts/social_browser_smoke.mjs`. Ele valida o contrato recorrente do roteiro de navegador social sem instalar dependência nova: controles de save/load na aba `Relações`, seletores e botões com `data-testid`, consequência em `WorldState` com metadados de `Barganhar`, penalidade T63 para `Pressionar`, save v4 com eventos de negociação e guia em ordem `Barganhar -> Fazer apelo -> WorldState -> Salvar sessão -> recarregue -> Carregar save`.
...
- `Pressionar` em negociação terminal aplica perda idempotente de 1 nível de `Fama` na facção do NPC, registrada por flag `WorldState` por encontro.
- Opções de diálogo bloqueáveis por HP mental: `Pressionar` exige HP mental 6 na corretora, HP mental 7 no informante e HP mental 8 no capitão, sem criar migration ou save v5.
...
- `Pressionar` reduz `Fama` da facção do NPC, mas ainda não altera `Infâmia`, relação individual por NPC ou clocks de retaliação.
