...
Este roteiro valida o MVP navegável atual do Pandorha Engine depois da T63. Ele não substitui `quality:gate`; ele descreve o caminho humano que deve ser repetido antes de expandir NPCs oficiais, diálogo literário completo ou consequências narrativas mais profundas.
...
13. Para validar T63, reinicie a negociação com a `Corretora de Treino`, escolha `Pressionar`, faça apelos até encerrar a conversa e confirme consequência com `perda de 1 nível de Fama` e `Fama` reduzida na `Liga Mercante de Treino`.
14. Clique em `Salvar sessão`, recarregue, clique em `Carregar save` e confirme que relações, negociação social, log com argumento, opção bloqueada, consequência com a escolha de diálogo e `Fama` reduzida foram restaurados.
