...
Este roteiro valida o MVP navegável atual do Pandorha Engine depois da T53. Ele não substitui `quality:gate`; ele descreve o caminho humano que deve ser repetido antes de expandir diálogo social em árvore, NPCs oficiais ou consequências narrativas mais profundas.
...
9. Na mesma aba, selecione `Corretora de Treino`, escolha o argumento `Barganhar`, confirme `Modificador do argumento: +1`, clique em `Iniciar negociação`, clique em `Fazer apelo` e confirme `Bônus 1`, `HP mental 5/8`, `Persuasão 1/3` e log citando `Barganhar`.
...
- seletor de `Argumento`, opção `Barganhar` e log com escolha social;
...
- Escolhas de argumento social (`Persuadir`, `Barganhar`, `Pressionar`) com modificador visível, log persistido e consequência em `WorldState`.
...
- A negociação social ainda usa escolhas simples de argumento; diálogo em árvore completo entra em tarefa futura.
