...
15. Repita `Pressionar` quando a `Fama` da facção já estiver em 0 e confirme que a consequência cita `Infâmia` e que `Relações` mostra um clock `Retaliação: Liga Mercante de Treino - 0/4 fatias`.
16. Clique em `Salvar sessão`, recarregue, clique em `Carregar save` e confirme que relações, negociação social, log com argumento, opção bloqueada, consequência, `Fama`, `Infâmia` e clock de retaliação foram restaurados.
...
- contrato T63-T70 de `Pressionar` com flag `social-pressure-fame-penalty`, perda de 1 nível de `Fama`, ganho de `Infâmia` quando a Fama já está em 0 e clock de retaliação salvo em `clocks`;
...
- `Pressionar` em negociação terminal aplica perda idempotente de 1 nível de `Fama` na facção do NPC; se a Fama já estiver em 0, aplica 1 nível de `Infâmia`.
- Pressão social que gera `Infâmia` ou `ultimatum` cria um clock ativo de retaliação salvo em `clocks`.
- Opções de diálogo bloqueáveis por HP mental, WorldState e Fama: `Pressionar` exige HP mental 6 na corretora, HP mental 7 no informante e HP mental 8 no capitão; `Barganhar` com o capitão exige Fama 1, sem criar migration ou save v5.
...
- `Pressionar` ainda não altera relação individual por NPC nem avança clocks de retaliação automaticamente.
